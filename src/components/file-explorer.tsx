...
const EditableCodeView = dynamic<{
  ...
  () => import("./code-view/editable-code-view").then((mod) => mod.EditableCodeView),
  { 
    ssr: false, 
    loading: () => (
...
const FileExplorer = ({ files, fragmentId, onSave, allowEdit = true }: FileExplorerProps) => {
  ...
  const handleSave = useCallback(async () => {
    if (!onSave || !hasUnsavedChanges || !fragmentId) {
      console.log("Save blocked:", { onSave: !!onSave, hasUnsavedChanges, fragmentId }); // Debug
      return;
    }
...
// interface FileExplorerProps {
//   files: FileCollection;
//   fragmentId: string; // ✅ fragmentId add korechi
//   onSave?: (fragmentId: string, files: FileCollection) => Promise<void>; // ✅ signature change
//   allowEdit?: boolean;
// }
...
//       setIsSaving(false);
//     }
//   }, [editedFiles, onSave, hasUnsavedChanges, fragmentId]); // ✅ dependency add
